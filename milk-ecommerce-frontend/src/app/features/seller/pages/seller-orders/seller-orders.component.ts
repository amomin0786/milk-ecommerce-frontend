...
export class SellerOrdersComponent implements OnInit {
  ...
  load(): void {
    ...
    this.ordersApi.sellerOrders().subscribe({
      next: (data: any[]) => {
        this.items = Array.isArray(data) ? data : [];
        this.items.sort((a, b) => Number(b?.id ?? 0) - Number(a?.id ?? 0));
        this.applyFilters();
        this.loading = false;
      },
      error: (e: any) => {
        this.loading = false;
        this.err = e?.error?.message || e?.error || 'Failed to load seller orders';
      }
    });
  }
  ...
  applyFilters(): void {
    ...
    this.filtered = this.items.filter((o: any) => {
  ...
  open(order: any): void {
    this.selected = order;
    this.newStatus = (order?.orderStatus || 'CONFIRMED') as OrderStatus;
    this.selectedItems = [];
    this.itemsError = '';
    ...
    const sellerItems = Array.isArray(order?.items) ? order.items : [];
    this.selectedItems = sellerItems;
  }
  ...
  saveStatus(): void {
    ...
    this.ordersApi.updateStatus(id, this.newStatus).subscribe({
      ...
      error: (e: any) => {
        this.saving = false;
        this.err = e?.error?.message || e?.error || 'Failed to update order status';
      }
    });
  }
  ...
  downloadCsv(): void {
    ...
    this.ordersApi.exportSellerReports(this.fromDate, this.toDate).subscribe({
      next: (data: Blob) => {
        ...
        a.href = url;
        a.download = `seller_orders_${this.fromDate}_to_${this.toDate}.csv`;
        ...
        this.success = 'Seller orders CSV downloaded successfully';
      },
      error: (e: any) => {
        this.exporting = false;
        this.err = e?.error?.message || e?.error || 'Failed to export seller orders';
      }
    });
  }
