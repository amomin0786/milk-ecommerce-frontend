...
export class ReportsComponent implements OnInit {
  ...
  load(): void {
    ...
    this.adminService.getReports(this.form.from, this.form.to).subscribe({
      next: (res: any) => {
        this.report = {
      ...
      error: (e: any) => {
        this.err = e?.error?.message || e?.error || 'Failed to load report';
        this.loading = false;
      }
    });
  }
  ...
  downloadCsv(): void {
    ...
    this.adminService.exportReports(this.form.from, this.form.to).subscribe({
      next: (data: Blob) => {
        ...
        a.href = url;
        a.download = `admin_report_${this.form.from}_to_${this.form.to}.csv`;
      ...
      error: (e: any) => {
        this.err = e?.error?.message || e?.error || 'Failed to export report';
        this.exporting = false;
      }
    });
  }
